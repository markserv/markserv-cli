...
const buildActiveMarkconf = (Markconf, confFile, confDir, importDepth) => {
  ...
  const deepSpider = (conf, pattern, ancestorOfMatch, trace, moduleType) => {
    for (const key in conf) {
      if ({}.hasOwnProperty.call(conf, key)) {
        ...
        if (memberType === 'object' && Array.isArray(memberType)) {
          memberType = 'array';
        }
        ...
        if (memberType === 'object') {
          ...
          const next = deepSpider(member, nextPattern, keyInPattern, trace, moduleType);
          if (next) {
            conf[key] = next;
          } else {
            delete conf[key];
  ...
  // const combinedConf = Object.assign({}, includers);

  const modifiers = collect('modifiers');
  console.log('\n\n\nMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM');
  console.log(modifiers);
  // const combinedConf = Object.assign({}, modifiers);
