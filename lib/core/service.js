	...
	const setWatches = () => new Promise((resolve, reject) => {
		...
		const restartMarkserv = changedFile => {
			markservService.pluginList.forEach(module => {
				// console.log(changedFile, module);
				// if (changedFile === module) {
					log.info(`Unload plugin: ${log.ul(module)}`);
					delete require.cache[module];
					// require.cache[module] = require(module);
				// }
			});
			...
			.then(() => {
				log.info(`Markconf.js file changed.`);
				sync.reload();
			});
		};

		// const reloadPlugin = changedFile => {
		// 	markservService.pluginList.forEach(module => {
		// 		// console.log(changedFile, module);
		// 		if (changedFile === module) {
		// 			log.info(`Unload plugin: ${log.ul(module)}`);
		...
		const handleChanges = (changedFile, changeType) => {
			...
			if (plugins.indexOf(changedFile) > -1) {
				...
				return;
			} else if (changedFile === MarkconfUrl ||
