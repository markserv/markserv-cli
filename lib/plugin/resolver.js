...
module.exports = config => {
	...
	const resolver = {
		_import: (pluginRef, type) => {
			console.log(pluginRef);
			console.log(subplug[type + 's'][pluginRef]);
			if (typeof subplug === 'object' &&
				typeof subplug[type + 's'] === 'object' &&
				typeof subplug[type + 's'][pluginRef] === 'object') {
				// const importedPlugin = subplug[type + 's'][pluginRef];
				// resolve(importedPlugin);

				const importedRequireName = subserv.MarkconfJs[type + 's'][pluginRef];
				console.log(importedRequireName);
				return importedRequireName;
				// const loaded = require(importedRequireName);
				// console.log(loaded);
				// return loaded;
			}

			const msg = `Could not import sub-Markconf plugin: ${log.hl(type)} > ${log.hl(pluginRef)}.`;
			log.error(msg);
			// reject(msg);
		},
		...
		_string: (name, type, key, def) => new Promise((resolve, reject) => {
			...
			try {
				...
				try {
					...
					try {
						const pluginRef = name.split('|')[2];
						name = resolver._import(pluginRef, type);
						pluginExpObj = require(name);
					} catch (err) {
						errors.push(err.toString());
