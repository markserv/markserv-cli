...
module.exports = (name, modulePath, ModifierModule, initFunction, markserv) => { // eslint-disable-line max-params
  ...
  const compileTemplate = newTemplate => {
    templateCompiler.compileTemplate(newTemplate || templatePath)
    .then(html => {
      ModifierModule.exports.template = html;
      log.trace(`Template compiled: ${log.ul(newTemplate || templatePath)}`);
    });
  };

  const updateTemplate = newTemplate => {
    ModifierModule.exports.template = newTemplate;
    compileTemplate(newTemplate);
  };

  if ({}.hasOwnProperty.call(meta, 'templatePath')) {
    updateTemplate(meta.templatePath);
  }
