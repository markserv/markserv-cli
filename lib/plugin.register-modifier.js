...
module.exports = (name, modulePath, ModifierModule, initFunction, markserv) => { // eslint-disable-line max-params
  ...
  const updateTemplate = (newTemplate, modifier) => {
    const pathToTemplate = newTemplate;
    templateCompiler.compileTemplate(newTemplate, ModifierModule)
    .then(html => {
