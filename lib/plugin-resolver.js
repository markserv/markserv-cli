...
const spider = (conf, pattern, childOfMatch, defs, dp, sb) => {
  for (const reference in conf) {
    const datum = conf[reference];
    const patternMatch = {}.hasOwnProperty.call(pattern, reference);
    const type = typeof datum;
    if (type === 'object') {
      sb.push(reference);

      const next = spider(datum, pattern[reference], patternMatch || childOfMatch, defs, dp, sb);
      conf[reference] = next;
    } else if (type === 'string') {
      // const activeModule = importModule(reference, datum, defs, dp);
      const modifier = loadModifier(reference, datum, defs.confDir, dp);
      if (modifier.module) {
        ...
        console.log(sb);
        conf = modifier.conf[sb[0]]; // <-- need to match these
        continue;
      }

      // const isModifier = verifyModifier(activeModule, reference, dp);
      // console.log(datum);
      // console.log(activeModule);
...
const buildActiveMarkconf = (Markconf, confFile, confDir, dp) => {
  // let ActiveMarkconf = generateImportPattern();
  ...
  const result = spider(Markconf, importPattern, false, defs, dp, []);
  // console.log('RRRRRRRRRRRRRRRRRRRRRRRRRRRRRR');
  // console.log(result);

  // const something = resolveModifiersBlock(ActiveMarkconf, defs, dp);
