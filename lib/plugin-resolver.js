...
const prepareModifierArray = (name, modifierAry, dir, depth) => {
  return new Promise((resolve, reject) => {
    ...
    for (const [index, item] of modifierAry.entries()) {
      ...
      if (itemType === 'string') {
        console.log('STRING STRING STRING STRING STRING STRING STRING STRING STRING STRING STRING ');
        promiseStack.push(prepareModifier(name, item, dir, depth).then(modifier => {
          if (modifier) {
            modifierArrayStack.push(modifier);
          }
          return modifierArrayStack;
        }));
      }
