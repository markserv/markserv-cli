...
module.exports = config => {
	...
	const resolver = {
		...
		_string: (name, type, key, def) => new Promise(resolve => {
		...
		if (Reflect.has(config.MarkconfJs, 'import')) {
			...
			.then(subMarkservService => {
				...
				resolvePlugins(config).then(finalConfig => {
					...
					// Inherit includers from subconf and pass to compiler
					const subIncluders = config.subconf.plugins.includers
					Reflect.ownKeys(subIncluders).forEach(includer => {
						if (!Reflect.has(config.plugins.includers, includer)) {
							config.plugins.includers[includer] = subIncluders[includer]
							console.log(config.plugins.includers[includer])
						}
					})
					config.compiler.setIncluders(config.plugins.includers)
