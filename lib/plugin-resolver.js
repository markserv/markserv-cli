  ...
  const deepSpider = (conf, pattern, ancestorOfMatch, trace) => {
    for (const reference in conf) { // eslint-disable-line guard-for-in
      ...
      // If we find a string within the allowable import pattern definition...
        ...
        //
        for (const stack of confMember) {
          for (const modifierN in stack) {
            if ({}.hasOwnProperty.call(stack, modifierN)) {
              const templateOverride = stack[modifierN];
              console.log(modifierN, templateOverride);

              const moduleName = modifierN;
              const modulePackage = modifierN;
              ...
              if (modifier.module) {
                ...
                if (validModifier) {
                  // Replace it with the active loaded node module
                  confMember[stack] = activeModifier.exports;
                } else {
                  confMember[stack] = null;
                }
              ...
              // If the string was an external Markconf
                ...
                // Replace node with the active loaded node module
                confMember[stack] = extraction[reference];
              }
              confMember[stack].overrideTemplate(templateOverride);
            }
          }
        }
      }
