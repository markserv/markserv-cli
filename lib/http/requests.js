...
const init = config => {
	const logHttpResponse = (payload, req, type) => {
		...
		const responseUrl = config.httpServerUrl + req.originalUrl
	...
	const handleRequest = (req, res) => {
		let requestPath = path.join(config.root, req.url)

		const isDir = helpFs.directoryExistsSync(requestPath)
		const isFile = helpFs.fileExistsSync(requestPath)

		// Make sure slash is on the end of a dir path for micromatch
		if (isDir && isLastCharInStr(requestPath, '/') === false) {
			requestPath += '/'
		}

		// Make sure relativeUrl can be opened by clicking from CLI
		let relativeUrl
		relativeUrl = path.relative(process.cwd(), path.resolve(requestPath))
		if (isDir) {
			relativeUrl = path.relative(process.cwd(), path.resolve(requestPath)) + '/'
		}
		if (relativeUrl === '/') {
		...
		log.info(chalk.cyan('Request: ') + log.ul(relativeUrl))
		...
		if (!exists) {
			...
			if (matchingModule.module) {
				handle(matchingModule, req.url, res, req)
				return
			}
		...
		const matchingModule = matchPath(req.url, config)
