...
const strap = ' Markserv ';
const loadingMsg = 'Loading...';
const loadedMsg = chalk.green('Loaded        ');
const colRamp = ['black', 'grey', 'white', 'green'];
...
const endInitialization = () => {
	if (!initializationEnded && activeLevel !== 'OFF') {
		const scrub = chars(scrubSeq, progressWidth + loadingMsg.length + extraPad + strap.length + 2);
		const progress = chars(blockFrames[blockFramesCount - 1], progressWidth + 2, 'green');
		const timerFrame = chalk.green.bold('✔');
		process.stdout.write(`${scrub}${chalk.bgGreen.black(strap)}  ${progress} ${timerFrame} ${loadedMsg}`);
...
const decorate = (level, message, id, importDepth) => {
	...
	if (LEVEL_ENUM[level.toUpperCase()] < LEVEL_ENUM[activeLevel] && !initializationEnded) {
		...
		scrub += chars(scrubSeq, progressWidth + loadingMsg.length + strap.length + extraPad + 2);
		...
		process.stdout.write(`${scrub}${strapCol(strap)}  ${progress} ${timerFrame} ${timerCol(loadingMsg)}`);
