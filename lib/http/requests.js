...
const init = (Markconf, plugins) => {
	...
	const handle = (pattern, requestPath, res, req) => {
		const modifier = plugins.modifiers[pattern];
	...
	const matchPath = url => {
		const patterns = plugins.modifiers;
	...
	const handleRequest = (req, res) => {
		...
		if (!isFile && !isDir && {}.hasOwnProperty.call(plugins.modifiers, 404)) {
