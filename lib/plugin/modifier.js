...
module.exports = props => {
	...
	log.trace('Registering modifier: ' + modPath);
	...
	const loadModifierTemplate = () => new Promise((resolve, reject) => {
		...
		templateCompiler.compileTemplate(url, ModMod)
		.then(html => {
	...
	const setTemplate = newTemplatePath => new Promise((resolve, reject) => {
		...
		templateCompiler.compileTemplate(newTemplatePath, ModMod)
		.then(html => {
	...
	const configure = conf => new Promise((resolve, reject) => {
		...
		const hasModifierTemplate = typeof ModMod.exports.templatePath === 'string';

		console.log(hasModifierTemplate);

		const hasMarkconfTemplate = typeof ModMod.exports.markconfTemplatePath === 'string';
		// const templateContent = ModMod.exports.template;

		// if (hasModifierTemplate === false || templateContent.length === 0) {
		...
		if (hasModifierTemplate && hasMarkconfTemplate) {
			...
			.then(() => {
				ModMod.exports.updateTemplate(ModMod.exports.markconfTemplatePath)
					.then(() => {
						resolve();
					});
			})
