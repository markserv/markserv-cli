...
const loadModule = (confName, moduleName, dir, importDepth) => {
  return new Promise((resolve, reject) => {
    ...
    const localAppFile = path.join(dir, 'Markconf.js');
    ...
    if (fs.fileExistsSync(localAppFile)) {
      log.trace('Modifier ' + log.ul(confName) + ' points to Markserv App: ' + log.ul(dir + confName) + '.', importDepth);

      module.exports(dir, importDepth).then(nextMarkconf => {
        const loaded = {
          conf: nextMarkconf
          // path: dir
        };
    ...
    try {
      ...
      const activeModule = require(moduleName);
      console.log(activeModule);

      log.trace('Found modifier: ' + log.ul(confName) + ' as NPM Module: ' + log.ul(dir) + '.', importDepth);
