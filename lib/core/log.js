...
log4js.configure({appenders: [{
	...
	layout: {
		type: 'pattern',
		pattern: strap + chalk.grey('%[%5.5p%]') + chalk.dim.grey(' ┇ ') + '%m'
	}}, {
		type: 'console',
		category: 'console-severe',
		layout: {
			type: 'pattern',
			pattern: strapErr + '%[%5.5p%] ┇ ' + chalk.red('%m')
		}
	}, {
		type: 'file',
...
const endInitialization = () => {
	if (!initializationEnded && activeLevel !== 'OFF') {
		...
		const progress = chars(blockFrames[blockFramesCount - 1], progressWidth + 1, 'green');
		const timerFrame = '';
