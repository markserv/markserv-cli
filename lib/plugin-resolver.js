...
const buildActiveMarkconf = (Markconf, confFile, dir, importDepth) => {
  ...
  if ({}.hasOwnProperty.call(conf, 'includers')) {
    const includers = loadIncluders(conf.includers, dir, importDepth);
    stack.includers = includers;
  }

  if ({}.hasOwnProperty.call(conf, 'modifiers')) {
    const modifiers = loadModifiers(conf.modifiers, dir, importDepth);
