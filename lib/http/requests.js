...
const init = config => {
	...
	const handle = (kind, requestPath, res, req) => {
		...
		if (definitionType === 'object') {
			...
			// console.log(Reflect.ownKeys(modifier))
			// console.log(modifier.name)

			modifier.handle(requestPath, res, req)
			.then(payload => {
				// Ignore null payloads (pipes, like files, ect)
	...
	const handleRequest = (req, res) => {
		...
		console.log('--------------------------------------')
		console.log(requestPath, matchingModule)
		console.log(isDir && matchingModule.module)
