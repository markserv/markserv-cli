...
const configure = (conf, activeIncluders) => {
	Markconf = conf;
	includers = activeIncluders;
};
...
const compileTemplate = (templateFilepath, modifier, include, node) => {
	...
	new Promise((resolve, reject) => {
		...
		if (filepath) {
			module.exports.compileTemplate(filepath, modifier, include)
			.then(content => {
	...
	return new Promise((resolve, reject) => {
		...
		// dry these next if blocks out...
		if (typeof include === 'undefined') {
			console.log(11111, 'undefined');
