...
const configFilepath = path.join(cwd, 'lib', pkg.settings.configFilename);
const defaultsFilepath = path.join(cwd, 'lib', pkg.settings.defaultsFilename);

let defaultsFileFound = helpFs.fileExistsSync(defaultsFilepath);

if (defaultsFileFound === false) {
  const defaultsFilepath = path.join(__dirname, 'lib', pkg.settings.defaultsfilename);
//   console.log(defaultsFilepath);
//   proces.exit(0);

  defaultsFileFound = helpFs.fileExistsSync(defaultsFilepath);
