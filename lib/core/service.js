...
const service = config => new Promise((resolve, reject) => {
	...
	const setWatches = () => new Promise((resolve, reject) => {
		...
		const browserSyncPort = Reflect.has(watchDef, 'port') && watchDef.port || 3001;
		...
		const browserSyncProxy = `${config.args.address}:${markservService.httpServer.port}`;
		...
		if (!sync.isActive()) {
			...
			sync.browserSyncServer = sync.start(syncConfig);
		}
