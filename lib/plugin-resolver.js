...
const loadModifier = (name, modifierModuleName, confDir, dp) => {
  ...
  if (fs.fileExistsSync(appFile)) {
    ...
    return module.exports(modPath, dp);
  }
...
const loadModifiersString = (modifierClass, ActiveMarkconf, defs, dp) => {
...
const loadModifiersObject = (modifierClass, ActiveMarkconf, defs, dp) => {
...
const resolveModifiers = (modifierClass, ActiveMarkconf, defs, dp) => {
  ...
  const modifierDeclarationType = typeof defs.Markconf.modifiers[modifierClass];

  switch (modifierDeclarationType) {
    case 'string':
      loadModifiersString(modifierClass, ActiveMarkconf, defs, dp);
      break;
    case 'object':
      loadModifiersObject(modifierClass, ActiveMarkconf, defs, dp);
      break;
    default:
      break;
  }
