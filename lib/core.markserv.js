...
const configurePlugins = Markconf => {
  ...
  // console.log(Markconf.plugins.modifiers);

  if (Markconf.plugins.modifiers) {
    for (const pluginName in Markconf.plugins.modifiers) {
      if ({}.hasOwnProperty.call(Markconf.plugins.modifiers, pluginName)) {
        log.trace('Configuring core modifier plugin: ' + log.hl(pluginName));
        const plugin = Markconf.plugins.modifiers[pluginName];
