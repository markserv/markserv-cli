	...
	return new Promise((resolve, reject) => {
		if (Reflect.has(config.MarkconfJs, 'import')) {
			...
			.then(subMarkservService => {
				...
				// resolvePlugins(config).then(() => {
				// 	if (Reflect.has(subplug, 'includers')) {
				// 		Reflect.ownKeys(subplug.includers).forEach(key => {
				// 			mergedPlugins.includers[key] = subplug.includers[key];
				// 		});
				// 	}

				// 	if (Reflect.has(config.plugins, 'includers')) {
				// 		Reflect.ownKeys(config.plugins.includers).forEach(key => {
				// 			mergedPlugins.includers[key] = config.plugins.includers[key];
				// 		});
				// 	}

				// 	if (Reflect.has(subplug, 'modifiers')) {
				// 		Reflect.ownKeys(subplug.modifiers).forEach(key => {
				// 			mergedPlugins.modifiers[key] = subplug.modifiers[key];
				// 		});
				// 	}

				// 	if (Reflect.has(config.plugins, 'modifiers')) {
				// 		Reflect.ownKeys(config.plugins.modifiers).forEach(key => {
				// 			mergedPlugins.modifiers[key] = config.plugins.modifiers[key];
				// 		});
				// 	}
