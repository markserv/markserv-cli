...
const spider = (conf, pattern, childOfMatch, defs, dp, trace) => {
  for (const reference in conf) { // eslint-disable-line guard-for-in
    ...
    if (type === 'object') {
      ...
      const next = spider(datum, pattern[reference], patternMatch || childOfMatch, defs, dp, trace);
      conf[reference] = next;
    } else if (type === 'string') {
      ...
      if (modifier.module) {
        ...
        const extraction = drill(trace, modifier.conf);
