...
const service = config => new Promise((resolve, reject) => {
	...
	const setWatches = markservService => new Promise((resolve, reject) => {
		...
		const plugins = Reflect.has(watch, 'plugins') && watch.plugins ? markservService.pluginList : false;
		const files = Reflect.has(watch, 'files') ? watch.files : false;

		// console.log(markservService);
		// console.log('--------------------------');
		// console.log(watch);
		...
		plugins.forEach(plugin => {
			watchList.push(path.join(plugin));
		});
		...
		const pluginChanged = plugin => new _Promise((resolve, reject) => {
			// stop http server
			markservService.pluginList.forEach(module => {
				log.info(`De-cache: ${log.ul(module)}`);
				delete require.cache[module];
			});

			liveReloadServer.close();
			log.info('The liveReloadServer was shutdown.');

			markservService.shutdown(markservService);
