...
const service = config => new Promise((resolve, reject) => {
	...
	const setWatches = () => new Promise((resolve, reject) => {
		...
		if (!sync.isActive()) {
			console.log('sync was not active');
		...
		const restartMarkserv = (changedFile, changeType) => {
			...
			watch.close();
			log.trace('The file watcher service was closed');

			markservService.initialize()
			.then(() => {
				log.trace('refresh:', changedFile);
				sync.reload();
			});
		};

		let handleChanges = (changedFile, changeType) => {
			...
			const changed = watchList.some(filePattern => {
				...
				log.info(`Watch: found rule ${log.hl(shortPattern)} for: ${log.hl(changeType)} ${log.ul(changedFile)}`);
		...
		// Lets only watch one path if all the paths are in the same tree
		if (relPath.length === 1) {
	...
	const isRootConf = () => {
		console.log(!Reflect.has(config, 'parent'));
		return !Reflect.has(config, 'parent');
	};

	const reportActive = () => new Promise(resolve => {
		// not working for some reason, getting double-logging of this event
		if (isRootConf()) {
