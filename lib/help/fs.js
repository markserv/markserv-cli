...
module.exports = config => {
	...
	const readfile = filepath => new Promise((resolve, reject) => {
		log.trace(`${log.hl('readfile')} helper reading file: ${log.ul(filepath)}.`);

		fs.readFile(filepath, 'utf8', (err, contents) => {
			if (err) {
				log.error(`${log.hl('readfile')} helper could not load file: ${log.ul(filepath)}.`);
				return reject(err);
			}

			log.trace(`${log.hl('readfile')} helper sucessfully read file: ${log.ul(filepath)}.`);
