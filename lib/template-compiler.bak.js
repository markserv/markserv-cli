...
const availableIncluder = include => {
  const plugin = Markconf.plugins.includers[include.includer];
...
const compileTemplate = (templateFilepath, modifier, include, templateHtmlBlock) => {
  let $DOM;
  ...
  const processNode = (node, include, dir) => {
    return new Promise((resolve, reject) => {
      ...
      if (filepath) {
        ...
        // console.log(ext);
        console.log(include);
        // console.log(Markconf.plugins.includers[include.includer]);

        const includerPlugin = availableIncluder(include);

        if (includerPlugin) {
          // console.log(includerPlugin);

          // includerPlugin.htmlCommentIncluder(include, dir, node).then(html => {
          //   module.exports.compileTemplate(filepath, modifier).then(content => {
          //   const $content = cheerio.load(html)._root;
