...
const loadModifier = (name, modifierModuleName, confDir, importDepth) => {
  ...
  // 2) Try require node module that is a deep dependancy of another Markconf
  try {
  ...
  // 3) Try straight-up node module 'require'
  try {
  ...
  const deepSpider = (conf, pattern, ancestorOfMatch, trace) => {
    for (const reference in conf) { // eslint-disable-line guard-for-in
      ...
      // If we find a string within the allowable import pattern definition...
        ...
        if (modifier.module) {
          const activeModifier = plugin(moduleName, modulePackage, modifier.module, modifier.path);
        ...
        for (const [index, elem] of confMember.entries()) {
          ...
          for (const moduleName in elem) {
            if ({}.hasOwnProperty.call(elem, moduleName)) {
              const templateOverride = elem[moduleName];
              ...
              conf[reference][index].overrideTemplate(templateOverride);
            }
          }
        }
      }
